...
class SwissProtXMLParser extends BaseXMLParser {
	...
	public $species = [];
	public $genes = [];
	public $organismSpecificGenes = [];
	public $attributes = [];
	public $ECNumbers = [];

	public $functionalDomains = [];
	public $similarityDomains = [];
	public $similarityFamilies = [];
	...
	protected function addComments( SwissProtEntry $entry, $definedMeaningId, $proteinMeaningId, $organismSpeciesMeaningId ) {
		foreach ( $entry->comments as $key => $comment ) {
			...
			case "SIMILARITY":
				...
				} elseif ( startsWith( $comment->text, "Contains " ) ) {
...
class CommentXMLElementHandler extends DefaultXMLElementHandler {
	...
	public function notify( $childHandler ) {
		...
		} elseif ( $childHandler instanceof IsoformXMLElementHandler ) {
			$this->comment->isoforms[] = $childHandler->isoform;
		} else { // there are different comments, so they need a different processing
...
class IsoformXMLElementHandler extends DefaultXMLElementHandler {
	...
	public function notify( $childHandler ) {
		switch ( $childHandler->name ) {
			case "ID":
				$this->isoform->id = $childHandler->data;
				break;
			case "NAME":
				$this->isoform->names[] = $childHandler->data;
				break;
			case "NOTE":
				$this->isoform->notes[] = $childHandler->data;
				break;
		}
	}
}
...
class SwissProtEntry {
	public $name = "";
	public $accession = "";
	public $secundaryAccessions = [];
	public $protein;
	public $EC = "";
	public $gene = "";
	public $geneSynonyms = [];
	public $organism = "";
	public $organismTranslations = [];
	public $comments = [];
	public $keywords = [];
	public $GOReference = [];
