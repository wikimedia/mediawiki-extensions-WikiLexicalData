...
class AddToCollection extends ApiBase {
	...
	public function execute() {
		...
		if ( !is_numeric( $this->collectionMid ) || !is_numeric( $this->definedMeaningId ) ) {
			if($ctr == 1) {
				$result = array ( 'note' => "either $this->collectionMid or $this->definedMeaningId is not an int or probably just the CSV header");
			} else {
				$result = array ( 'note' => "either $this->collectionMid or $this->definedMeaningId is not an int");
			}
		} else {
	...
	public function processBatch( $wikiPage ) {
		...
		while ( $inputData = array_shift( $inputLine ) ) {
			...
			if ( $inputDataCount <> 3 ) {
				$result = array ( 'note' => "The line `$inputData` can not be processed because it has $inputDataCount columns instead of 3.");
				$this->getResult()->addValue( null, $this->getModuleName(),
					array ( 'result' . $ctr => $result )
				);
				continue;
