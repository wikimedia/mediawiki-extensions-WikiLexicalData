...
var Fat = { make_hex: function ( r, g, b ) {
	...
	var rf = parseInt( _23.substr( 1, 2 ), 16 ),
		gf = parseInt( _23.substr( 3, 2 ), 16 ),
		bf = parseInt( _23.substr( 5, 2 ), 16 ),
		rt = parseInt( to.substr( 1, 2 ), 16 ),
		gt = parseInt( to.substr( 3, 2 ), 16 ),
		bt = parseInt( to.substr( 5, 2 ), 16 ),
		r, g, b, h;
	while ( _28 < _25 ) {
...
if ( wFORMS ) {
	...
	wFORMS.behaviors.hint = { name: 'hint', evaluate: function ( _40 ) {
		...
		var _43 = wFORMS.helpers.getSourceElement( e ),
			_44 = document.getElementById( _43.id + wFORMS.idSuffix_fieldHint );
		if ( !_44 ) {
		...
		var _46 = wFORMS.helpers.getSourceElement( e ),
			_47 = document.getElementById( _46.id + wFORMS.idSuffix_fieldHint );
		if ( !_47 ) {
...
if ( wFORMS ) {
	...
	wFORMS.behaviors.validation = { evaluate: function ( _72 ) {
		...
		var _93 = document.createTextNode( ' ' + _92 ),
			fe = document.getElementById( _91.id + wFORMS.idSuffix_fieldError );
		if ( !fe ) {
...
if ( wFORMS ) {
	...
	wFORMS.behaviors.paging = { evaluate: function ( _97 ) {
		...
		var _a1 = _a0.parentNode,
			_a2 = parseInt( _a1.id.replace( /[\D]*/, '' ) ) + 1,
		...
		var _a9 = _a8.parentNode,
			_aa = parseInt( _a9.id.replace( /[\D]*/, '' ) ) - 1,
			_ab = document.getElementById( wFORMS.idPrefix_pageIndex + _aa.toString() );
		if ( _ab ) {
	...
	wFORMS.behaviors.repeat = { evaluate: function ( _af ) {
		if ( wFORMS.helpers.hasClass( _af, wFORMS.className_repeat ) ) {
			...
			if ( !_b0 ) {
				...
				var _b1 = document.createElement( 'span' ),
					_b2 = document.createTextNode( wFORMS.arrMsg[ 0 ] );
		...
		if ( wFORMS.helpers.hasClass( _af, wFORMS.className_delete ) ) {
			var _b7 = document.createElement( 'a' ),
				_b1 = document.createElement( 'span' ),
				_b2 = document.createTextNode( wFORMS.arrMsg[ 2 ] );
		...
		if ( _b9 ) {
			...
			var _bb = parseInt( counterField.value ) + 1,
				_bc = '-' + _bb.toString(),
				_bd = wFORMS.behaviors.repeat.replicateTree( _b9, null, _bc, _ba ),
				_be = _b9.nextSibling;
