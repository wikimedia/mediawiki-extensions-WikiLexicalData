...
class Question {
	 ...
	 * 			(for easy callback).
	 * @param DOMElement $questionNode	a raw DOMElement with our defined meaning
	 */
	...
	public function getQuestionDefinitions( $languages = null ) {
		if ( $languages === null ) {
			$languages = $this->getQuestionLanguages();
		}
		if ( $languages === null ) {
			throw new Exception( "No language specified" );
		}

		$definitions = []; # _typically_ there's only  one, but this is not enforced anywhere, afaik.
	...
	public function getAnswers( $languages = null ) {
		if ( $languages === null ) {
			$languages = $this->getAnswerLanguages();
		}
		if ( $languages === null ) {
			throw new Exception( "No language specified" );
		}
